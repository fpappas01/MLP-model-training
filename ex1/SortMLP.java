...
public class SortMLP {
    ...
    public SortMLP(int d, int k, int h1, int h2, int h3, double learningRate, double terminationThreshold, int B) {
        this.d = d;
        this.k = k;
        this.h1 = h1;
        this.h2 = h2;
    ...
    public double[] forwardPass(Example x) {
        ...
        for (int i = 1; i < layers.length - 1; i++) {
            ...
            for (int j = 0; j < neurons.length; j++) {
                ...
                neurons[j].setValue(tahnAF(preActivationVal + neurons[j].getBias()));
                updateConnectionsValues();
            }
        }
    ...
    private void calculateHiddenLayerErrors(Layer layer) {
        ...
        for (int i = 0; i < neurons.length; i++) {
            // Here it is 1 - tahn(u)^2 , but tahn(u) = neuron value after activation
            error = (1 - Math.pow(neurons[i].getValue(), 2))  * sumProductsOfNextLayer(neurons[i], layer.getNext());
    ...
    private void initializeData() {
        ...
        Map<String, ArrayList<Point>> sdt = dataCreator.getDataSDT();
    ...
    private Example[][] getRandomExamples(Map<String, ArrayList<Point>> sdt) {
